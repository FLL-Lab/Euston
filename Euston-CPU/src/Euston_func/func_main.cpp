...
int main(int argc, char* argv[])
{
	...
	if (!has_b || !has_m || !has_p) {
		...
		if (!has_p) std::cerr << "  -p/--protocol [0/1/2/3/4/5/6]" << std::endl;
		if (!has_b) std::cerr << "  -b/--batchsize [32/64/128/256/512]" << std::endl;
		if (!has_m) std::cerr << "  -m/--model [BERT/GPT/LLAMA]" << std::endl;
    }
	if(model == "BERT")
	{
	...
	if(TEST_TARGET == TEST_TARGETS[0])
	{
		...
		double size_store = cal_pt_size(hat_R);
		cout << "Client offline Storage: " << size_store << " MB" << endl;
		double size_off_commun = 0;
		size_off_commun += cal_pt_size(hat_U_cpack);
		size_off_commun += cal_pt_size(hat_V);
		size_off_commun += cal_ct_size(hat_D_ct);
		cout << "Client offline Communication: " << size_off_commun << " MB" << endl;
		ckks.print_chain_index(hat_D_ct, "D_ct");
		...
		double size_on_commun = cal_pt_size(hat_X);
		cout << "Online Phase amortized communication size: " << size_on_commun / 32 << " MB" << endl;
		timer.start();
		...
		{
			errors += ckks.calculate_errors(cipher_output[i], output_cpack[i]);
		}
		cout << fixed << setprecision(10) << "Average errors: " << errors / output_cpack.size() << endl;
	}
	else if(TEST_TARGET == TEST_TARGETS[1])
	{
		...
		vector<vector<Ciphertext>> K_cpack_rotates(matrix_input_cpack.size());
		timer.start();
	...
	else if(TEST_TARGET == TEST_TARGETS[2])
	{
		...
		timer.start();
		vector<vector<Ciphertext>> right_matrix_input_cpack_rotates(right_matrix_input_cpack.size());
	...
	else if(TEST_TARGET == TEST_TARGETS[3])
	{
		...
		CKKSEvaluator ckks_client_mat = construct_ckks_evaluator_matrix(MATRIX_COEFF_MODULI);
		CKKSEvaluator ckks = ckks_client_mat;
		matrix left_matrix_input = loadtxt(dname+"/input/CPMM1_"+model+"_left_input_32x128X"+to_string(dim_model_weight_in)+".txt");
		matrix left_matrix_input_cpack = mat_transpose(left_matrix_input);
		matrix right_matrix_input = loadtxt(dname+"/input/CPMM1_"+model+"_right_input_"+to_string(dim_model_weight_in)+"X64.txt");
		matrix right_matrix_input_cpack = mat_transpose(right_matrix_input);
		matrix output = loadtxt(dname+"/output/CPMM1_"+model+"_output_32x128X64.txt");
		...
		Rectime timer;
		timer.start();
		cipherMatrix cipher_output_cpack = cMat_mul_pMat(&ckks, cipher_left_matrix_input_cpack, right_matrix_input_cpack);
	...
	else if(TEST_TARGET == TEST_TARGETS[6])
	{
		...
		timer.elapsed("加密耗时：");
		// ckks.print_dec(cipher_QKV_ln_cpack[0], "x:cipher_QKV_ln_cpack[0]", 5);
		...
		for(int i = 0; i < col_siz; i++)
		{
			...
			Ciphertext cipher_signs = cipher_sign(&ckks_sign, ct, 3, 3, 1); 
		...
		for(int i = 0; i < col_siz; i++)
		{
			ckks.evaluator->mod_switch_to_inplace(gelu_rectify[i], gelu[i].parms_id());
