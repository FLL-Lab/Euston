    ...
    */
    ...
    public:
        ...
        */
		...
		这段文本描述了 Galois 元素在加密算法中的作用，特别是在同态加密中。以下是该段文字的中文翻译：

		---

		**Galois 元素**是在区间 [1, M-1] 内的奇数，其中  
		M = 2 * N，且 N = poly_modulus_degree。与批处理一起使用时，Galois 元素  
		3^i % M 对应于向左循环移动 i 步的行旋转，而  
		Galois 元素 3^(N/2-i) % M 对应于向右循环移动 i 步的行旋转。  
		Galois 元素 M-1 对应于 BFV 中的列旋转（行交换），以及 CKKS 中的复共轭操作。  
		在多项式视图下（不使用批处理），通过 Galois 元素 p 进行的 Galois 自同构会将  
		Enc(plain(x)) 转换为 Enc(plain(x^p))。

		---

		这段话解释了如何使用 Galois 元素来实现行旋转、列旋转（行交换）、复共轭等操作，同时提到了其在不同同态加密方案（如 BFV 和 CKKS）中的具体作用。
