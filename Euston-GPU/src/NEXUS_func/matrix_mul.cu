...
void MMEvaluator::matrix_mul(vector<vector<double>> &x, vector<vector<double>> &matrix_weight_T, vector<PhantomCiphertext> &res) {
	matrix y;
	...
	int b_cts_count = dim_model_weight_in * dim_model_weight_out / ckks->degree;
	...
	for (int i = 0; i < dim_model_weight_out; i++) {
		...
		vector<PhantomCiphertext> temp_cts(dim_model_weight_in);
